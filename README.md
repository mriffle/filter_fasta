...
1. **Parses a Percolator XML results file** and applies two independent q-value filters:
...
```bash
python3 filter_fasta.py -f <input.fasta> -x <percolator.xml> [-o <output.fasta>] [--psm-qvalue <cutoff>] [--peptide-qvalue <cutoff>]
```
...
| `-x`, `--xml` | Yes | — | Percolator results XML file |
